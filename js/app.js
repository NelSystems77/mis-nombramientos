  ...
  addHoraExtraField(data = null) {
    ...
    div.innerHTML = `
      <div class="form-group">
        <label>Fecha</label>
        <input type="date" name="horaExtra_fecha_${id}" value="${data?.fecha || ''}" required>
      </div>
      <div class="form-group">
        <label>Horas</label>
        <input type="number" name="horaExtra_horas_${id}" min="0.5" step="0.5" value="${data?.horas || ''}" required>
      </div>
      <button type="button" class="hora-extra-remove" onclick="this.closest('.hora-extra-item').remove()">
        🗑️
      </button>
    `;
