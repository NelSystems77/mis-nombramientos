...
class ReportsManager {
  ...
  async generatePDF(tipo, periodo, fechaDesde = null, fechaHasta = null) {
    try {
      ...
      // Agregar nombre del usuario si existe
      if (profile && profile.name) {
        doc.text(`Usuario: ${profile.name}`, 105, yPos, { align: 'center' });
        yPos += 5;
      }
      
      doc.text(`Generado: ${new Date().toLocaleDateString('es-CR')}`, 105, yPos, { align: 'center' });
  ...
  async generateExcel(tipo, periodo, fechaDesde = null, fechaHasta = null) {
    try {
      ...
      if (tipo === 'general') {
        this.addGeneralExcelSheet(wb, stats, profile);
      } else if (tipo === 'detallado') {
        this.addDetailedExcelSheet(wb, stats, profile);
      } else if (tipo === 'horas') {
        this.addHorasExcelSheet(wb, stats, profile);
      } else if (tipo === 'lugares') {
        this.addLugaresExcelSheet(wb, stats, profile);
      }
    ...
    try {
      ...
      let html = `
        <div class="report-header">
          ...
          ${profile && profile.name ? `<p><strong>Usuario:</strong> ${profile.name}</p>` : ''}
  ...
  // Excel Sheets
  addGeneralExcelSheet(wb, stats, profile = null) {
    const wsData = [
    ...
    if (profile && profile.name) {
      wsData.push(['Usuario:', profile.name]);
    }
  ...
  addDetailedExcelSheet(wb, stats, profile = null) {
    const wsData = [
  ...
  addHorasExcelSheet(wb, stats, profile = null) {
    const nombramientosConHoras = stats.nombramientos.filter(n => 
      n.horasExtras && n.horasExtras.length > 0
    );
  ...
  addLugaresExcelSheet(wb, stats, profile = null) {
    const wsData = [
